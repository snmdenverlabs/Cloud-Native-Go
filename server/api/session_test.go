...
func TestSessionToJSON(t *testing.T) {
	session := Session{ID: "07092020-09", Date: "07/09/2020", Guest: "Rev. Sudhir Dureja Ji", Comments: "Pure Bliss!", GuestWish: "Rev. Nand Ahuja Ji"}
	json := session.ToJSON()

	assert.Equal(t, `{"_id":"07092020-09","gchurcha_date":"07/09/2020","guest_speaker":"Rev. Sudhir Dureja Ji","liked_most":"Pure Bliss!","wish_list_gspeaker":"Rev. Nand Ahuja Ji"}`, string(json), "Session JSON marshalling wrong.")
}
